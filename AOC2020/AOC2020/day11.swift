...
//  AOC2020
//
//  Created by Ryan Johnstone on 11/12/2020.
//
...
func day11() {
    ...
    func part1() {
        ...
        for (index, line) in lines.enumerated() {
            ...
            for (charIndex, char) in line.enumerated() {
                let space = Space(x: charIndex, y: index, state: State(rawValue: "\(char)")!)
                spaces.append(space)
            }
            spaces2d.append(spaces)
        }

        update(spaces2d: spaces2d)
    }
    ...
    func part2() {
        ...
        update(spaces2d: spaces2d, partTwo: true)
    }

    func update(spaces2d: [[Space]], partTwo: Bool = false) {

        var nextRoundSpaces2d: [[Space]] = []
        var changed = false
        for spaces in spaces2d {
            var nextRoundSpaces: [Space] = []
            for space in spaces {
                var spaceCopy = space
                let neighbours = partTwo ? getNextSeats(spaces: spaces2d, x: space.x, y: space.y) : getNeighbours(spaces: spaces2d, x: space.x, y: space.y)
                let filledNeighbours = neighbours.filter { $0.state == .filled }

                switch space.state {
                case .empty:
                    if filledNeighbours.count == 0 {
                        changed = true
                        spaceCopy.state = .filled
                    }
                case .filled:
                    if filledNeighbours.count >= (partTwo ? 5 : 4) {
                        changed = true
                        spaceCopy.state = .empty
                    }
                ...
                nextRoundSpaces.append(spaceCopy)
            }

            nextRoundSpaces2d.append(nextRoundSpaces)
        }

        printSpaces(spaces: nextRoundSpaces2d)

        if (!changed) {
            var occupied = 0
            nextRoundSpaces2d.map {
                $0.map {
                    if $0.state == .filled {
                        occupied += 1
                    }
            ...
            update(spaces2d: nextRoundSpaces2d, partTwo: partTwo)
        }
    }
    ...
    func getNeighbours(spaces: [[Space]], x: Int, y: Int) -> [Space] {
        var neighbours: [Space] = []
        for moveX in [-1, 0, +1] {
            for moveY in [-1, 0, +1] where !(moveX == 0 && moveY == 0)  {
                let newX = x + moveX
                let newY = y + moveY
                guard spaces.indices.contains(newY) else { continue }
                guard spaces[newY].indices.contains(newX) else { continue }
                neighbours.append(spaces[newY][newX])
            }
        }
        return neighbours
    }

    func getNextSeats(spaces: [[Space]], x: Int, y: Int) -> [Space] {
        var neighbours: [Space] = []
        for moveX in [-1, 0, +1] {
            for moveY in [-1, 0, +1] where !(moveX == 0 && moveY == 0)  {
                if let space = find(x: x, y: y, spaces: spaces, direction: (x: moveX, y: moveY)) {
                    neighbours.append(space)
                }
            }
        }
        return neighbours
    ...
    func find(x: Int, y: Int, spaces: [[Space]], direction: (x: Int,y: Int)) -> Space? {
        var multiplier = 1
        ...
        while true {
            ...
            guard spaces.indices.contains(newY) else { return nil }
            guard spaces[newY].indices.contains(newX) else { return nil }

            if spaces[newY][newX].state != .floor {
                return spaces[newY][newX]
            }
